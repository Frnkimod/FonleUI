...
namespace ArduinoAudioPlayer
{
    public class Tool
    {
        ...
        public double GetLong(int i,int remove, int transformTo,int gain)
        {
            float[] newAveraged = FFT.AvgValues(GetFFTData(BASSData.BASS_DATA_FFT256,256), transformTo, gain);
            double FFTdata = (newAveraged[remove + i]);
            return FFTdata;
        }
